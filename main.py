...
def convert_to_blit_coords(x, y):
    if not planet_locked:
        x = ((
                         x - width / 2 + lock_offset_x + camera_offset_x + new_planet.sprite.get_width() / 2) / zoom) + rel_mouse_x - new_planet.sprite.get_width() / 2 / zoom
        y = ((
                         y - height / 2 + lock_offset_y + camera_offset_y + new_planet.sprite.get_height() / 2) / zoom) + rel_mouse_y - + new_planet.sprite.get_height() / 2 / zoom
    else:
        x = ((
                         x - width / 2 + lock_offset_x + new_planet.sprite.get_width() / 2) / zoom) + rel_mouse_x - new_planet.sprite.get_width() / 2 / zoom
        y = ((
                         y - height / 2 + lock_offset_y + new_planet.sprite.get_height() / 2) / zoom) + rel_mouse_y - + new_planet.sprite.get_height() / 2 / zoom
    return x, y
...
def create_collided_planet(body1, body2):
    ...
    amount_of_debris = round(25 * impact_violence)
    if amount_of_debris > 0:
        ...
        for x in range(amount_of_debris):
            ...
            '''
            debris_vel_x = vel_x + ((debris_x - pos_x) * 0.000000023) + random.uniform(-1, 1) * 0.0000004
            debris_vel_y = vel_y + ((debris_y - pos_y) * 0.000000023) + random.uniform(-1, 1) * 0.0000004
